import { formatPhoneNumber, normalizePhoneDigits, isTollFreePhone, borrowerPhoneFromCallerId } from './phone.js';
import { unwrapZoomBody } from './zoom-payload.js';
import { splitFullName } from './name.js';
import { searchShapeLeadByPhone } from './shape/search-lead-by-phone.js';
import {
  assignShapeLeadOwner,
  createShapeInboundLead,
  updateShapeLeadFields,
} from './shape/client.js';
import { resolveInboundLo } from './shape/inbound-lo-roster.js';
import { upsertLeadFromShapeCall, createPendingCallLead } from './leads.js';
import { findTranscriptByExternalCallId, insertInitialCallTranscript } from './transcripts.js';
import { buildDispositionEmail } from './disposition-email.js';
import { resolveInboundCallChannel } from './mailer/resolve-channel.js';
import { resolveQuestMailAtCallAnswered } from './mailer/call-routing.js';
import {
  applyQuestMailCallbackToChannel,
  findQuestMailCallbackContext,
  resolveQuestMailFromCallbackContext,
} from './mailer/questmail-callback.js';

/**
 * True when Zap sends raw Zoom phone.callee_answered payload (no pre-built shape_lead_id).
 */
export function isZoomCallAnsweredPayload(body) {
  if (!body || typeof body !== 'object') return false;
  if (body.shape_lead_id || body.shapeLeadId) return false;
  if (body.mode === 'legacy') return false;
  if (body.mode === 'full' || body.pipeline === 'full') return true;
  if (body.event === 'phone.callee_answered') return true;
  if (body.payload?.object?.caller || body.payload?.object?.callee) return true;
  return false;
}

/**
 * Parse Zoom callee_answered webhook into a normalized call context.
 */
export function parseZoomCallAnswered(body) {
  const zoom = unwrapZoomBody(body);
  const payloadObject = body.payload?.object ?? {};

  const caller = payloadObject.caller ?? {};
  const callee = payloadObject.callee ?? {};
  const acceptedBy = payloadObject.accepted_by ?? payloadObject.acceptedBy ?? {};
  const owner = payloadObject.owner ?? {};

  const callerPhone = zoom.callerPhone ?? caller.phone_number ?? body.caller_phone;
  const calleeName =
    zoom.calleeName ??
    callee.name ??
    owner.name ??
    body.callee_name ??
    body.accepted_by_name;
  const calleeExtension =
    callee.extension_number ??
    callee.extensionNumber ??
    owner.extension_number ??
    owner.extensionNumber ??
    body.callee_extension ??
    body.callee_extension_number;
  const calleePhone = zoom.calleePhone ?? callee.phone_number ?? body.callee_phone;
  const acceptedByName =
    acceptedBy.name ?? body.accepted_by_name ?? body.accepted_by?.name ?? null;
  const acceptedByExtension = acceptedBy.extension_number
    ? String(acceptedBy.extension_number).replace(/\D/g, '')
    : acceptedBy.extensionNumber
      ? String(acceptedBy.extensionNumber).replace(/\D/g, '')
      : '';

  const timestamp =
    zoom.timestamp ??
    payloadObject.answer_start_time ??
    payloadObject.ringing_start_time ??
    body.answer_start_time ??
    new Date().toISOString();

  const callId =
    zoom.callId ??
    payloadObject.call_id ??
    payloadObject.callId ??
    body.call_id ??
    `answered-${Date.now()}-${normalizePhoneDigits(callerPhone) || 'unknown'}`;

  const callerName = zoom.callerName ?? body.caller_name ?? '';
  const split = splitFullName(callerName);

  return {
    event: body.event ?? 'phone.callee_answered',
    callId: String(callId),
    callerPhone,
    callerName,
    callerFirstName: split.firstName,
    callerLastName: split.lastName,
    calleeName,
    calleeExtension: calleeExtension ? String(calleeExtension).replace(/\D/g, '') : '',
    calleePhone,
    acceptedByName,
    acceptedByExtension,
    timestamp: new Date(timestamp).toISOString(),
    forwardedByName: payloadObject.forwarded_by?.name ?? body.forwarded_by_name ?? null,
  };
}

/**
 * Full call-answered pipeline: Shape search/create → assign owner → Supabase → disposition email.
 * QuestMail toll-free: hold until transcript identifies borrower — no Shape create at answer.
 */
export async function runCallAnsweredPipeline(supabase, body) {
  const zoom = parseZoomCallAnswered(body);

  const formattedPhone = formatPhoneNumber(zoom.callerPhone);
  const phoneDigits = normalizePhoneDigits(zoom.callerPhone);

  const dedupeKey = `${zoom.callId}:answered`;
  const existingTranscript = await findTranscriptByExternalCallId(supabase, dedupeKey);
  if (existingTranscript) {
    return {
      success: true,
      skipped: true,
      reason: 'duplicate_call',
      call_id: zoom.callId,
      transcript_id: existingTranscript.transcript_id,
    };
  }

  let channelInfo = resolveInboundCallChannel(zoom, body);
  let isQuestMail = channelInfo.channel === 'questmail';
  let questMailCallbackContext = null;
  let shapeSearch = null;

  if (!isQuestMail && formattedPhone && phoneDigits.length === 10) {
    shapeSearch = await searchShapeLeadByPhone(zoom.callerPhone);
    questMailCallbackContext = await findQuestMailCallbackContext(supabase, {
      phoneDigits,
      shapeLeadId: shapeSearch?.found ? shapeSearch.leadId : null,
    });
    if (questMailCallbackContext) {
      channelInfo = applyQuestMailCallbackToChannel(channelInfo, questMailCallbackContext);
      isQuestMail = true;
    }
  }

  if (!isQuestMail) {
    if (!formattedPhone || phoneDigits.length !== 10) {
      const error = new Error('Borrower phone must contain at least 10 digits.');
      error.statusCode = 400;
      throw error;
    }
  } else if (!formattedPhone && phoneDigits.length < 10) {
    const error = new Error('QuestMail call must include a valid caller phone (often toll-free).');
    error.statusCode = 400;
    throw error;
  }

  let contactFound = false;
  let shapeLeadId = null;
  let firstName = zoom.callerFirstName || 'WIRELESS';
  let lastName = zoom.callerLastName || 'CALLER';
  let email = '';
  let questMailResolution = null;
  let questmailDeferred = false;

  if (isQuestMail) {
    const callbackResolution = questMailCallbackContext
      ? resolveQuestMailFromCallbackContext(questMailCallbackContext)
      : null;

    questMailResolution =
      callbackResolution ??
      (await resolveQuestMailAtCallAnswered(supabase, {
        phoneDigits,
        firstName,
        lastName,
        formattedPhone,
        callerPhoneRaw: zoom.callerPhone,
      }));

    questmailDeferred = Boolean(questMailResolution.deferred);
    shapeLeadId = questMailResolution.shapeLeadId;
    contactFound = questMailResolution.contactFound;

    if (questMailResolution.mailerLead?.first_name) {
      firstName = questMailResolution.mailerLead.first_name;
    }
    if (questMailResolution.mailerLead?.last_name) {
      lastName = questMailResolution.mailerLead.last_name;
    }
    if (questMailResolution.mailerLead?.email) {
      email = questMailResolution.mailerLead.email;
    }

    if (isTollFreePhone(zoom.callerPhone)) {
      firstName = questMailResolution.mailerLead?.first_name || 'QuestMail';
      lastName = questMailResolution.mailerLead?.last_name || 'Caller';
    }
  } else {
    const search = shapeSearch ?? (await searchShapeLeadByPhone(zoom.callerPhone));
    if (search.found && search.leadId) {
      contactFound = true;
      shapeLeadId = search.leadId;
      firstName = search.firstName || firstName;
      lastName = search.lastName || lastName;
      email = search.email || '';
    } else {
      const created = await createShapeInboundLead({
        firstname: firstName,
        lastname: lastName,
        phone: phoneDigits,
      });

      if (!created.created || !created.shape_lead_id) {
        const error = new Error(created.error ?? 'Failed to create Shape lead');
        error.statusCode = 502;
        error.details = created;
        throw error;
      }

      shapeLeadId = created.shape_lead_id;
    }
  }

  const lo = resolveInboundLo({
    calleeName: zoom.calleeName,
    calleeExtension: zoom.acceptedByExtension || zoom.calleeExtension,
    calleePhone: zoom.calleePhone,
    acceptedByName: zoom.acceptedByName ?? zoom.forwardedByName,
  });

  /** QuestMail + inbound Shape search miss → email after transcript AI. */
  const pendingDisposition = questmailDeferred || (!isQuestMail && !contactFound);

  let ownerAssign = { synced: false, skipped: true, reason: 'LO not resolved' };
  if (shapeLeadId && lo?.depursLo) {
    ownerAssign = await assignShapeLeadOwner(shapeLeadId, lo.depursLo);
  } else if (questmailDeferred) {
    ownerAssign = { synced: false, skipped: true, reason: 'QuestMail deferred until transcript' };
  }

  const fullName = [firstName, lastName].filter(Boolean).join(' ').trim() || 'Unknown Caller';

  let lead;
  let leadCreated;
  let mailerMatched = null;

  if (questmailDeferred) {
    const borrowerPhone = borrowerPhoneFromCallerId(zoom.callerPhone);
    const pending = await createPendingCallLead(supabase, {
      fullName,
      phoneNumber: borrowerPhone,
      email: email || null,
      leadSource: 'questmail',
      referenceCode: questMailResolution?.mailerLead?.reference_code ?? null,
    });
    lead = pending.lead;
    leadCreated = pending.created;
  } else {
    const upserted = await upsertLeadFromShapeCall(supabase, {
      shapeLeadId,
      fullName,
      phoneNumber: formattedPhone,
      email: email || null,
      leadSource: isQuestMail ? 'questmail' : 'inbound_zoom',
      referenceCode: questMailResolution?.mailerLead?.reference_code ?? null,
    });
    lead = upserted.lead;
    leadCreated = upserted.created;
    mailerMatched = upserted.mailerMatched;
  }

  const { transcript, created: transcriptCreated } = await insertInitialCallTranscript(supabase, {
    lead,
    externalCallId: zoom.callId,
    timestamp: zoom.timestamp,
    callSource: isQuestMail ? 'QuestMail' : 'Zoom Phone',
    callMeta: {
      event: 'call_answered',
      call_id: zoom.callId,
      call_channel: channelInfo.channel,
      shape_marketing_source: channelInfo.shapeSourceId,
      questmail_state: channelInfo.questmail?.state ?? null,
      questmail_label: channelInfo.questmail?.label ?? null,
      questmail_type: channelInfo.questmail?.mailerType ?? null,
      questmail_toll: channelInfo.questmail?.phone10 ?? null,
      channel_matched_by: channelInfo.matchedBy ?? null,
      questmail_callback_matched_by: channelInfo.callbackMatchedBy ?? null,
      landing_page_state: channelInfo.landing?.state ?? null,
      landing_page_label: channelInfo.landing?.label ?? null,
      utm_campaign: channelInfo.utmCampaign ?? null,
      dialed_number: channelInfo.dialedNumber ?? zoom.calleePhone ?? null,
      lo_name: lo?.displayName ?? null,
      lo_email: lo?.email ?? null,
      lo_extension: lo?.extension ?? zoom.calleeExtension ?? null,
      lo_phone: lo?.phone10 ?? null,
      depurs_lo: lo?.depursLo ?? null,
      disposition_id: lo?.dispositionId ?? null,
      contact_found: contactFound,
      reference_code: questMailResolution?.mailerLead?.reference_code ?? null,
      questmail_hold: questmailDeferred,
      pending_disposition: pendingDisposition,
      caller_is_toll_free: isTollFreePhone(zoom.callerPhone),
      questmail_resolution_reason: questMailResolution?.reason ?? null,
    },
  });

  let shapeSync = { skipped: true, reason: 'QuestMail deferred until transcript' };
  if (shapeLeadId) {
    const shapeFieldsToSync = { phone: formattedPhone };
    if (firstName) shapeFieldsToSync.firstname = firstName;
    if (lastName) shapeFieldsToSync.lastname = lastName;
    if (email) shapeFieldsToSync.email = email;
    shapeSync = await updateShapeLeadFields(shapeLeadId, shapeFieldsToSync);
  }

  let dispositionEmail = null;
  if (lo?.email && shapeLeadId && !pendingDisposition) {
    dispositionEmail = buildDispositionEmail({
      leadId: shapeLeadId,
      firstName,
      lastName,
      leadPhone: phoneDigits,
      lo,
      callTime: zoom.timestamp,
      baseUrl: body.disposition_base_url ?? process.env.DISPOSITION_BASE_URL,
    });
  }

  return {
    success: true,
    pipeline: 'full',
    call_channel: channelInfo.channel,
    shape_marketing_source: channelInfo.shapeSourceId,
    questmail_state: channelInfo.questmail?.state ?? null,
    questmail_deferred: questmailDeferred,
    contact_found: contactFound,
    pending_disposition: pendingDisposition,
    shape_lead_id: shapeLeadId,
    call_id: zoom.callId,
    formatted_phone: formattedPhone,
    first_name: firstName,
    last_name: lastName,
    full_name: fullName,
    lo: lo
      ? {
          display_name: lo.displayName,
          email: lo.email,
          depursLo: lo.depursLo,
          matched_by: lo.matchedBy,
          extension: lo.extension,
        }
      : null,
    owner_assign: ownerAssign,
    lead_id: lead.lead_id,
    lead_created: leadCreated,
    transcript_id: transcript.transcript_id,
    transcript_created: transcriptCreated,
    shape_sync: shapeSync,
    mailer_matched: mailerMatched ?? questMailResolution?.mailerLead ?? null,
    questmail_resolution: questMailResolution,
    disposition_email: dispositionEmail,
    zoom: {
      callee_name: zoom.calleeName,
      callee_extension: zoom.calleeExtension,
      forwarded_by: zoom.forwardedByName,
    },
  };
}
