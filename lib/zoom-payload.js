...
import { resolveLoFromPhone } from './shape/lo-phone-routing.js';
...
export function normalizePayload(body) {
  ...
  let loName = String(
    zoom.loName ?? body.lo_name ?? body.loName ?? body.accepted_by_name ?? '',
  ).trim() || undefined;

  if (!loName) {
    const calleeExtension = zoom.calleeExtension ?? body.callee_extension;
    const fromPhone = resolveLoFromPhone(calleeExtension || zoom.calleePhone || body.callee_phone);
    if (fromPhone?.loName) {
      loName = fromPhone.loName;
    }
  }

  const callerName = String(
    zoom.callerName ?? body.caller_name ?? nameFromParts ?? '',
  ).trim() || undefined;

  const calleeName = String(
    zoom.calleeName ?? body.callee_name ?? loName ?? '',
  ).trim() || undefined;
  ...
  return {
    ...
    callId: zoom.callId,
    callerName,
    callerPhone: zoom.callerPhone ?? body.caller_phone,
    calleeName,
    calleePhone: zoom.calleePhone ?? body.callee_phone,
    direction: zoom.direction,
    timestamp: zoom.timestamp,
    transcriptText: zoom.transcriptText,
    loName: loName ?? calleeName,
    fullName,
    email: zoom.email ?? body.email,
