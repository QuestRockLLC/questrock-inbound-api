import { formatPhoneNumber, isTollFreePhone } from './phone.js';
...
import { isShapeTrackerCallId, shapeLeadIdFromCallId } from './call-tracker/call-id.js';
...
export async function resolveLeadContextForCall(supabase, { callId, callerPhone, direction = 'inbound' }) {
  if (callId) {
    if (isShapeTrackerCallId(callId)) {
      const created = await findTranscriptByExternalCallId(supabase, `${callId}:created`);
      if (created?.lead_id) {
        ...
        if (!error && lead) {
          const meta = created.fields_populated ?? {};
          return {
            lead,
            shapeLeadId: shapeLeadIdFromCallId(callId) || lead.shape_lead_id || null,
            resolvedBy: 'shape_created',
            callChannel: meta.call_channel ?? 'shape_inbound',
          };
        }
      }
    }
    ...
    const transcript = await findTranscriptByExternalCallId(supabase, `${callId}:transcript`);
    if (transcript?.lead_id) {
      ...
      if (!leadError && lead) {
        const questmail = lead.lead_source === 'questmail';
        return {
          lead,
          shapeLeadId: lead.shape_lead_id ?? null,
          resolvedBy: 'transcript_row',
          questmailPending: questmail && !lead.shape_lead_id,
          callChannel: questmail ? 'questmail' : 'inbound_zoom',
        };
      }
    }
  }
  ...
  if (formattedPhone && !isTollFreePhone(formattedPhone)) {
