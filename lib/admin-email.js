import { sanitizeLoCoachingDisplay } from './ai/ray-coaching-format.js';
import { REVIEW_META_KEYS } from './transcript-ai-review.js';
...
export function buildAdminOutcomeEmail({
  ...
  const email_to = process.env.ADMIN_NOTIFICATION_EMAIL || 'arashid@questrock.com';
  ...
  const populatedEntries = Object.entries(evaluation.fieldsPopulated ?? {}).filter(
    ([key, value]) =>
      ...
      String(value ?? '').trim() !== '' &&
      !String(value).includes('[SSN REDACTED]') &&
      !String(value).includes('[DOB REDACTED]'),
  );
...
<body style="font-family:Helvetica,Arial,sans-serif;background:#f3f4f6;padding:24px;">
  <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
    ...
    <div style="padding:24px;">
      ...
      ${evaluation.salesNotes ? `<p style="margin:0 0 8px;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">LO Coaching — QuestRock AI</p><pre style="margin:0 0 20px;color:#374151;line-height:1.55;white-space:pre-wrap;font-family:Helvetica,Arial,sans-serif;background:#fffbeb;padding:14px;border-radius:8px;border-left:4px solid #f59e0b;">${escapeHtml(sanitizeLoCoachingDisplay(evaluation.salesNotes))}</pre>` : ''}
      ...
      <p style="margin:0 0 20px;color:#374151;line-height:1.6;">${
        shapeSync?.synced
          ...
          : shapeSync?.skipped
            ? escapeHtml(
                shapeSync.reason ||
                  'Shape sync skipped — API key not configured or transcript sync disabled.',
              )
