...
/**
 * Inserts the missed-call row (unanswered inbound).
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 */
export async function insertMissedCallTranscript(
  supabase,
  { lead, externalCallId, timestamp, callSource, callMeta },
) {
  const existing = await findTranscriptByExternalCallId(supabase, `${externalCallId}:missed`);
  ...
  const hash = createInitialCallHash({
    externalCallId: `${externalCallId}:missed`,
    leadId: lead.lead_id,
  });
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      ai_status_color: lead.current_status_color,
      fields_populated: callMeta ?? null,
      external_call_id: `${externalCallId}:missed`,
    })
    .select('*')
    .single();
...
/**
 * Synthetic Call Tracker row when a lead is created in Shape (no Zoom call yet).
 */
export async function insertShapeLeadCreatedEvent(
  supabase,
  { lead, shapeLeadId, timestamp, callMeta = {} },
) {
  const shapeId = String(shapeLeadId ?? '').trim();
  if (!shapeId || !lead?.lead_id) {
    return { transcript: null, created: false };
  }

  const externalCallId = `shape:${shapeId}:created`;
...
export async function appendTranscript(supabase, input) {
  ...
  const rawText = String(input.transcriptText ?? '');
