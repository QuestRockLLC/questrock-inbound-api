import { createShapeLeadViaPost, assignShapeLeadOwner } from '../shape/client.js';
  ...
  for (const entry of rows) {
    ...
    try {
      ...
      if (shapeResult.created && shapeResult.shape_lead_id) {
        ...
        const conciergeAssign = await assignShapeLeadOwner(
          shapeResult.shape_lead_id,
          SHAPE_CONCIERGE_USER_ID,
        );

        if (!conciergeAssign.synced && !conciergeAssign.skipped) {
          summary.errors.push({
            row: index,
            reference_code: row.reference_code,
            stage: 'shape_concierge',
            error: conciergeAssign.error || 'Concierge owner assign failed',
            shape_assignment: conciergeAssign,
          });
        } else if (conciergeAssign.synced) {
          await supabase
            .from('mailer_leads')
