...
import { assignConciergeAfterShapeCreate } from './concierge-assign.js';
import { getMailerImportStatus } from './import-status.js';
import { buildShapeLeadPayload } from './normalize.js';
import { upsertLeadForMailerRow } from './upsert-lead.js';

const CONCIERGE_BATCH_OPTS = { initialDelayMs: 100, retries: 2, retryDelayMs: 250 };
...
export async function importMailerRowToDatabase(supabase, entry, { batchId, mailerStatus, existingMailer }) {
  ...
  const mailerPatch = {
    ...
    curr_pay_date: row.curr_pay_date || null,
    new_pay_date: row.new_pay_date || null,
  ...
  for (const entry of rows) {
    ...
    try {
      ...
      if (shapeResult.created && shapeResult.shape_lead_id) {
        ...
        const conciergeOutcome = await assignConciergeAfterShapeCreate(
          supabase,
          shapeResult.shape_lead_id,
          mailerLead.mailer_lead_id,
          CONCIERGE_BATCH_OPTS,
        );

        if (conciergeOutcome.assigned) {
          summary.concierge_assigned += 1;
        } else if (!conciergeOutcome.skipped) {
          summary.concierge_failed += 1;
          summary.errors.push({
            row: index,
            reference_code: row.reference_code,
            stage: 'shape_concierge',
            error:
              conciergeOutcome.result?.error ||
              conciergeOutcome.result?.reason ||
              'Concierge owner assign failed after retries',
            shape_assignment: conciergeOutcome.result,
          });
        }
...
export async function importMailerRows(supabase, options) {
  ...
  const mailerStatus = await getMailerImportStatus(supabase);
