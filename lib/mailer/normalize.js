 ...
 */
import { buildMailerShapeNotes } from './mailer-notes.js';
import { MAILER_IMPORT_STATUS_LABEL } from './import-status.js';
...
export const MAILER_FIELD_ALIASES = {
  ...
  lender: ['lender'],
  loan_type: ['type', 'loan type', 'mail type', 'product type'],
  rate_type: ['rate type', 'ratetype'],
  new_rate: ['new rate'],
  new_apr: ['new apr', 'newapr'],
  debt_amount: ['debtamt', 'debt amount'],
  new_total_payment: ['newtotpay', 'new total pay', 'new total payment', 'new pi pay', 'new_pi_pay'],
  mail_date: ['mail date', 'maildate'],
  offer_expires: ['expir', 'expires', 'expiration', 'offer expires'],
  curr_pay_date: ['curr pay', 'current pay'],
  new_pay_date: ['new pay', 'new pay date'],
...
export function buildMailerNotes(row) {
  return buildMailerShapeNotes(row);
}
...
export function buildShapeLeadPayload(row) {
  ...
  const payload = {
    ...
    mstrstatus1: MAILER_IMPORT_STATUS_LABEL,
  };
