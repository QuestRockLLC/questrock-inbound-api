...
export async function processQuestMailTranscriptPipeline(payload) {
  ...
  const match = await matchMailerLeadFromTranscript(
    ...
    {
      referenceCode: payload.referenceCodeHint ?? lead.reference_code ?? null,
    },
  );
  ...
  if (!match.matched || !match.mailerRow) {
    ...
    const error = new Error(
      'QuestMail call could not be matched to an imported mailer lead (offer code or callback phone). Import the mailer row or confirm the borrower stated their offer code / cell phone on the call.',
    );
