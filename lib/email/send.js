 ...
 *
 * Fields are sent in the JSON body and mirrored on the hook URL query string (except large HTML).
 *
 * Env: ZAPIER_EMAIL_WEBHOOK_URL (or legacy ZAPIER_ADMIN_NOTIFY_WEBHOOK_URL)
 */
import { postJsonToZapier } from './zapier-webhook.js';
...
async function sendViaZapier({ to, cc, subject, html, text, template, meta }) {
  ...
  const response = await postJsonToZapier(url, payload, {
