...
const BUTTON_STYLES = {
  advanced: { bg: '#16A34A', border: '#15803D', icon: '&#x2713;', title: 'Advanced', sub: 'Moving forward' },
  did_not_advance: { bg: '#EAB308', border: '#CA8A04', icon: '&#x23F8;&#xFE0F;', title: 'Did Not Advance', sub: 'Stalled — no path' },
  not_contacted: { bg: '#F59E0B', border: '#D97706', icon: '&#x1F4F5;', title: 'Not Contacted', sub: 'Voicemail / no conversation' },
  turndown: { bg: '#DC2626', border: '#B91C1C', icon: '&#x1F6AB;', title: 'Turndown', sub: 'Not interested' },
  bad_lead: { bg: '#B91C1C', border: '#991B1B', icon: '&#x2715;', title: 'Bad Lead', sub: 'Dead / wrong number' },
  help_requested: { bg: '#7C3AED', border: '#6D28D9', icon: '&#x1F6A8;', title: 'Help Requested', sub: 'Needs assistance' },
};

/** Only these six LO disposition options appear in email (per QuestRock ops). */
const BUTTON_GROUPS = [
  { title: 'Green — Moving Forward', slugs: ['advanced'] },
  { title: 'Yellow — Hold', slugs: ['did_not_advance', 'not_contacted'] },
  { title: 'Red — Dead / Denied', slugs: ['turndown', 'bad_lead'] },
  { title: 'Purple — Urgent', slugs: ['help_requested'] },
];

function renderButton(slug, href, suggestedSlug) {
  const style = BUTTON_STYLES[slug];
  const meta = STATUS_SLUG_META[slug];
...
export function buildDispositionEmail({
  ...
  const lo_email = String(lo.email ?? '').trim();
...
<p style="margin:0 0 12px 0;font-size:15px;font-weight:700;color:#111827;">${escapeHtml(suggestedMeta.label)}</p>
<p style="margin:0;font-size:13px;color:#6B7280;line-height:1.5;">Tap the highlighted button if you agree. Otherwise pick the status that fits.</p>
  ...
  const buttonSections = BUTTON_GROUPS.map((group) => {
    const cells = group.slugs
      .map((slug, index) => {
        const width = group.slugs.length === 1 ? '100%' : '49%';
        const pad =
          group.slugs.length === 1
            ...
            : index === 0
              ? 'padding-right:6px;'
              : 'padding-left:6px;';
        return `<td width="${width}" style="${pad}">${renderButton(slug, link(slug), suggestedSlug)}</td>`;
      })
      .join('');
...
<tr><td align="center" style="padding-top:20px;"><p style="margin:0;font-size:11px;color:#9CA3AF;">Powered by Zoom + QuestRock</p></td></tr>
  ...
  const email_from_display =
    lo.displayName && lo_email ? `${lo.displayName} <${lo_email}>` : lo_email || lo.displayName || '';
  ...
  return {
    ...
    email_to: lo_email,
    email_from: lo_email,
    email_from_name: lo.displayName,
    email_from_display,
