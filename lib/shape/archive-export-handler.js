...
export async function handleShapeArchiveExport(req, res) {
  ...
  try {
    ...
    if (action === 'import_notes' || action === 'notes_from_bulk') {
      if (!batchId) {
        return sendJson(res, 400, { ok: false, error: 'batch_id required for import_notes.' });
      }

      const result = await runImportNotesFromBulkChunk(supabase, batchId, {
        maxLeads: Math.min(Number(body.max_leads ?? body.maxLeads) || 40, 60),
      });
