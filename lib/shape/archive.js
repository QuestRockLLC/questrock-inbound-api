...
async function upsertArchiveNotes(supabase, noteRows) {
  if (!noteRows.length) {
    return 0;
  }
  ...
  if (!String(error.message ?? '').includes('ON CONFLICT')) {
    throw error;
  }
...
async function persistLeadNotes(supabase, batchId, archiveLead, leadId, leadPayload) {
  ...
  let inserted = await upsertArchiveNotes(supabase, noteRows);
...
export async function runImportNotesFromBulkChunk(supabase, batchId, {
  ...
  const { data: leads, error: leadsError } = await supabase
    ...
    .select('*')
    .eq('batch_id', batchId)
    .in('enrich_status', ['pending', 'failed', 'done'])
...
async function importTranscriptNotes(supabase, batchId, archiveLead, leadId) {
  ...
  for (const transcript of transcripts ?? []) {
    ...
    inserted += await upsertArchiveNotes(supabase, [row]);
  }
