...
import { DEFAULT_BULK_EXPORT_FIELDS, fetchShapeBulkExportPage } from '../shape/bulk-export.js';
...
function noteRowsFromLead(batchId, archiveLeadId, shapeLeadId, leadId, lead) {
  ...
  const sidebar = String(lead.notes_sidebar ?? '').trim();
  if (sidebar) {
  ...
  const aiNote = String(lead.notes_sidebar_ai_note ?? '').trim();
  if (aiNote) {
    rows.push({
      batch_id: batchId,
      archive_lead_id: archiveLeadId,
  ...
  const recent = String(lead.recent_notes ?? '').trim();
  if (recent) {
    rows.push({
      batch_id: batchId,
      archive_lead_id: archiveLeadId,
    ...
    try {
      ...
      if (noteRows.length) {
        const { error: notesError } = await supabase
          .from('shape_archive_notes')
          .upsert(noteRows, { onConflict: 'external_id' });

        if (notesError) {
          throw notesError;
        }

        notesAdded += noteRows.length;
      }

      const transcriptNotes = await importTranscriptNotes(
        supabase,
        batchId,
        updatedLead,
        leadId,
      );
      notesAdded += transcriptNotes;
