...
import { looksLikeTranscriptLeak } from '../deal-review/deny-list.js';
...
async function importTranscriptNotes(supabase, batchId, archiveLead, leadId) {
  ...
  for (const transcript of transcripts ?? []) {
    const fields = transcript.fields_populated ?? {};
    const noteText =
      String(fields.notes_sidebar ?? '').trim() ||
      (fields.deal_review?.borrower_objective
        ? `Deal Review: ${String(fields.deal_review.borrower_objective).slice(0, 500)}`
        : '');

    if (!noteText || looksLikeTranscriptLeak(noteText)) {
      continue;
    }
