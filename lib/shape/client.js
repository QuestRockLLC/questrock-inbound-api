...
/**
 * Assigns LO owner on a Shape lead (depursLo = LOA user id).
 */
export async function assignShapeLeadOwner(shapeLeadId, depursLo) {
  const { apiKey, updateUrl } = getShapeConfig();
  ...
  const leadid = Number(shapeLeadId);
  const loUserId = Number(depursLo);
  ...
  if (Number.isNaN(loUserId) || loUserId <= 0) {
    return {
      synced: false,
      skipped: true,
      reason: `Invalid depursLo user id: ${depursLo}`,
    };
  }
  ...
  if (!response.ok) {
    return {
      ...
      shape_response: shapeResponse,
      depursLo: loUserId,
      error: `Shape LO assign rejected (${response.status})`,
      update_url: updateUrl,
    };
  }
