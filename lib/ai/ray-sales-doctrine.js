/**
 * QuestRock sales training — canonical doctrine for QuestRock AI.
...
export function buildRayDoctrineContext() {
  ...
  return `QUESTROCK SALES DOCTRINE (operationalize in every call review):
...
QUESTROCK AI OUTCOME CODES (assign on first calls when possible):
...
PITCH / ADVANCED CALLS (when past first call): Diagnose → Design → Decide. Authority framing → 3 C's → calendar control. Benefits stack when numbers discussed: rate/P&I/PITI as RANGES, skipped payments, escrow refund, PMT×8 framing, principal payoff math, program fit. QuestRock delivery: calm advisor, one idea at a time, conversational — not stacked hype.
...
export function buildRayAnalysisGuidance() {
  return `Apply QuestRock sales doctrine to questrock_analysis fields:
...
• next_steps_and_status — QuestRock AI outcome code (A-E) if first call. Exact calendar action. Shape task implied. If turndown — salvage path before accepting dead.`;
}
