/** Structured Ray LO coaching sections — stored in fields_populated.ray_coaching */

export const RAY_COACHING_UI_SECTIONS = [
  ['ONE-LINE VERDICT', 'one_line_verdict'],
  ['CALL PHASE', 'call_phase'],
  ['RAY OUTCOME CODE', 'ray_outcome_code'],
  ...
  ['RAY SCRIPT TO DEPLOY', 'ray_script'],
...
export function formatRayCoachingText(rayCoaching) {
  ...
  return RAY_COACHING_UI_SECTIONS.filter(([, key]) => String(rayCoaching[key] ?? '').trim())
