...
const RESPONSE_FORMAT = {
  ...
  json_schema: {
    ...
    schema: {
      ...
      required: ['status_label', 'status_rationale', 'call_summary', 'sales_notes', 'ops_notes', 'lead_fields', 'extracted_fields'],
      additionalProperties: false,
    },
  },
};
...
export async function evaluateCallWithAi({
  ...
  const salesNotes = String(parsed.sales_notes ?? '').trim();
  const opsNotes = String(parsed.ops_notes ?? '').trim();

  if (salesNotes) {
    fieldsPopulated.notes_sidebar = salesNotes;
  }
