...
export async function pasteCallTranscript(
  ...
  let anchor = await findTranscriptByExternalCallId(
    supabase,
  ...
  if (!anchor?.lead_id && existing?.lead_id && !shapeCall) {
    ...
    if (leadRow) {
      ...
      anchor = await findTranscriptByExternalCallId(supabase, `${normalizedCallId}:answered`);
    }
  }
  ...
  if (!anchor?.lead_id) {
    ...
    if (context?.lead) {
      anchor = await findTranscriptByExternalCallId(
        supabase,
        `${normalizedCallId}${callAnchorSuffix(normalizedCallId)}`,
      );
    }
  ...
  if (!anchor?.lead_id) {
    const error = new Error(
      shapeCall
        ? `No Shape arrival row for ${normalizedCallId}. Run shape-lead-sync or wait for the next cron.`
  ...
  const anchorMeta = anchor.fields_populated ?? {};
  const timestamp = anchor.timestamp ?? new Date().toISOString();
  const callSource = shapeCall
    ? 'Shape CRM'
    : anchorMeta.call_channel === 'questmail' || context.questmailPending
      ? 'QuestMail'
      : 'Zoom Phone';
  ...
  const { transcript, created } = await appendTranscript(supabase, {
    ...
    fieldsPopulated: {
      pasted_from_zoom: !shapeCall,
      paste_source: 'call_tracker',
      ...(shapeCall ? { pasted_for_shape_arrival: true } : {}),
    },
  });
  ...
  if (!shapeLeadId && !context.questmailPending && anchorMeta.call_channel !== 'questmail') {
  ...
  const result = await runBackgroundTranscriptJob({
    ...
    lead: context.lead,
    questmailPending:
      context.questmailPending ?? (anchorMeta.call_channel === 'questmail' && !shapeLeadId),
    callId: normalizedCallId,
    transcriptText: text,
    timestamp,
    loName: anchorMeta.lo_name ?? null,
    formattedPhone: context.lead.phone_number,
    fullName: context.lead.full_name,
    referenceCodeHint: context.lead.reference_code || anchorMeta.reference_code || null,
  });
