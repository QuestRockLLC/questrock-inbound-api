...
export async function refetchCallTranscript(supabase, callId) {
  ...
  if (!answered?.lead_id) {
    await ensureCallAnsweredRow(supabase, {
      ...
      callSource: questmailPending || context.lead.lead_source === 'questmail' ? 'QuestMail' : 'Zoom Phone',
      callMeta: {
        call_channel: questmailPending || context.lead.lead_source === 'questmail' ? 'questmail' : 'inbound_zoom',
        backfilled_from: 'call_tracker_refetch',
