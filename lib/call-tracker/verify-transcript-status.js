...
export function suggestStatusFromTranscript(text, signals = null) {
  ...
  if (found.includes('callback_scheduled') && found.includes('intake_facts')) {
    return {
      status: 'First Call Appointment Scheduled',
      confidence: 0.85,
      reason: 'Intake facts collected and a callback was scheduled (not a pitch appointment).',
    };
  }
...
export function verifyAiStatusAgainstTranscript({ aiStatusLabel, transcriptText, callSummary } = {}) {
  ...
  if (
    ...
    detectIntakeFactsCollected(combined) &&
    !signals.includes('bankruptcy_disqualified') &&
    !signals.includes('bankruptcy_seasoning')
  ) {
    if (detectExplicitPitchScheduled(combined)) {
      reportStatus = 'Application completed · Pitch scheduled';
    } else if (detectCallbackScheduled(combined)) {
      reportStatus = 'Application completed · Callback scheduled';
    }
  }
