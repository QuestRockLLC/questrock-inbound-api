...
export function suggestStatusFromTranscript(text, signals = null) {
  ...
  if (found.includes('callback_scheduled')) {
    return {
      ...
      confidence: 0.85,
      reason: 'A follow-up callback was scheduled — no application completed on this call.',
    };
  }
...
export function verifyAiStatusAgainstTranscript({ aiStatusLabel, transcriptText, callSummary } = {}) {
  ...
  if (
    ...
    if (detectExplicitPitchScheduled(combined) && detectIntakeFactsCollected(combined)) {
      reportStatus = 'Application completed · Pitch scheduled';
    } else if (detectCallbackScheduled(combined)) {
      reportStatus = 'Callback scheduled';
    }
  }
