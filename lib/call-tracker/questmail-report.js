...
import { resolveQuestMailCycle } from './questmail-cycle.js';
import { auditQuestMailTollLines } from './questmail-toll-audit.js';
...
const GOOD_STATUSES = new Set([
  ...
  'application completed · callback scheduled',
]);
...
export function categorizeQuestMailOutcome(call) {
  ...
  if (opsStatus === 'Application completed · Callback scheduled') {
    return {
      bucket: 'good',
      bucket_label: 'App completed · callback scheduled',
      reason: opsStatus,
    };
  }
...
function firstGistSentence(text, maxLen = 180) {
...
function gistFromTranscriptText(text, maxLen = 180) {
  ...
  return firstGistSentence(cleaned, maxLen);
}

/** Short call gist for report tables — AI summary first, then transcript excerpt. */
function buildTranscriptGist(call) {
  const summary = call.call_summary?.trim();
  if (summary) {
    return firstGistSentence(summary);
  }

  const rationale = call.status_rationale?.trim();
  if (rationale) {
    return firstGistSentence(rationale);
  }
  ...
  if (sales) {
    const line = sales
      ...
      .find(Boolean);
    if (line) {
      return firstGistSentence(line);
    }
  }

  const ops = call.ops_notes?.trim();
  if (ops) {
    return firstGistSentence(ops);
  }

  return gistFromTranscriptText(call.transcript_text) || 'No transcript yet.';
}
...
function buildWhatsNext(call) {
  ...
  if (opsStatus === 'Application completed · Callback scheduled') {
    return 'Callback scheduled — continue intake or send app on the follow-up call.';
  }
...
function enrichCall(call) {
  ...
  return {
    ...
    transcript_gist: buildTranscriptGist(call),
...
function buildSummary(rows) {
  ...
  for (const row of rows) {
    ...
    if (row.ops_status_label === 'Application completed · Callback scheduled') {
      counts.app_completed_callback_scheduled = (counts.app_completed_callback_scheduled ?? 0) + 1;
    }
...
/**
 * QuestMail ops report for the current mailer cycle (default Jun 16–23).
 */
export async function buildQuestMailReport(supabase, cycleOptions = {}) {
  const cycle = resolveQuestMailCycle(cycleOptions);
  ...
  const toll_audit = await auditQuestMailTollLines(supabase, {
    since: cycle.since,
    until: cycle.until,
  });
  ...
  return {
    ...
    lead_cycle: buildLeadCycleReport(calls, { cycleLabel: cycle.label }),
    toll_audit,
    calls,
    count: calls.length,
  };
