...
export function categorizeQuestMailOutcome(call) {
  ...
  if (opsStatus === 'Callback scheduled') {
    return {
      bucket: 'good',
      bucket_label: 'Callback scheduled',
      reason: opsStatus,
    };
  }
...
function buildWhatsNext(call) {
  ...
  if (opsStatus === 'Callback scheduled') {
    return 'Callback scheduled — follow up on the next call; no application completed yet.';
  }
...
function buildSummary(rows) {
  ...
  for (const row of rows) {
    ...
    if (row.ops_status_label === 'Callback scheduled') {
      counts.callback_scheduled = (counts.callback_scheduled ?? 0) + 1;
    }
