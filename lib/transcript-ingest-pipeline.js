...
export async function runTranscriptIngestPipeline(supabase, body, { asyncMode = false } = {}) {
  ...
  if (existing) {
    ...
    if (context?.lead) {
      await ensureCallAnsweredRow(supabase, {
        callId: parsed.callId,
        lead: context.lead,
        timestamp: existing.timestamp ?? parsed.timestamp,
        callSource: context.lead.lead_source === 'questmail' ? 'QuestMail' : 'Zoom Phone',
        callMeta: {
          call_channel: context.lead.lead_source === 'questmail' ? 'questmail' : 'inbound_zoom',
          backfilled_from: 'duplicate_transcript_webhook',
  ...
  const questmailPending = context.questmailPending ?? false;
  const callChannel =
    questmailPending || context.lead.lead_source === 'questmail' ? 'questmail' : 'inbound_zoom';
  ...
  await ensureCallAnsweredRow(supabase, {
    ...
    callSource: callChannel === 'questmail' ? 'QuestMail' : 'Zoom Phone',
    callMeta: {
      call_channel: callChannel,
      lo_name: lo?.displayName ?? null,
      lo_email: lo?.email ?? null,
      lo_extension: lo?.extension ?? null,
      depurs_lo: lo?.depursLo ?? null,
      disposition_id: lo?.dispositionId ?? null,
      contact_found: Boolean(context.shapeLeadId),
      questmail_hold: questmailPending,
      pending_disposition: questmailPending,
      shape_lead_id: context.shapeLeadId ?? context.lead.shape_lead_id ?? null,
      reference_code: context.lead.reference_code ?? null,
    },
  });
