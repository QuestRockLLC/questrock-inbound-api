 ...
 */
export async function runTranscriptIngestPipeline(supabase, body, { asyncMode = true } = {}) {
  ...
  if (asyncMode) {
    ...
    return {
      ...
      message: 'Transcript saved; AI evaluation runs in background.',
    };
  }

  const result = await runBackgroundTranscriptJob(pipelinePayload);
  return { success: true, pipeline: 'full', async: false, ...result };
}
