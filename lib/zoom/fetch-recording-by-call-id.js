...
function normalizeCallLogRow(data, callId) {
  const acceptedBy = data?.accepted_by ?? data?.acceptedBy ?? {};
  const owner = data?.owner ?? {};
  const callee = data?.callee ?? {};

  return {
    call_id: String(data?.call_id ?? data?.callId ?? data?.id ?? callId),
    caller_name: data?.caller_name ?? data?.callerName ?? null,
    caller_phone: data?.caller_number ?? data?.callerNumber ?? data?.caller?.phone_number ?? null,
    callee_name: data?.callee_name ?? data?.calleeName ?? owner.name ?? callee.name ?? null,
    callee_extension: String(
      data?.callee_number ??
    ...
    callee_phone: data?.callee_number ?? data?.calleeNumber ?? callee.phone_number ?? null,
    accepted_by_name: acceptedBy.name ?? data?.accepted_by_name ?? null,
    accepted_by_extension: String(acceptedBy.extension_number ?? acceptedBy.extensionNumber ?? '').replace(
      /\D/g,
      '',
    ),
    owner_type: owner.type ?? null,
    owner_name: owner.name ?? null,
    owner_extension: String(owner.extension_number ?? owner.extensionNumber ?? '').replace(/\D/g, ''),
    timestamp: data?.date_time ?? data?.dateTime ?? data?.start_time ?? new Date().toISOString(),
    direction: data?.direction ?? 'inbound',
  };
}

/** Build phone.callee_answered body for runCallAnsweredPipeline. */
export function buildCallAnsweredPayloadFromCallLog(log) {
  const object = {
    call_id: log.call_id,
    direction: log.direction || 'inbound',
    answer_start_time: log.timestamp,
    caller: {
      phone_number: log.caller_phone,
      name: log.caller_name,
    },
    callee: {
      name: log.callee_name || log.owner_name,
      extension_number: log.callee_extension || log.owner_extension,
      phone_number: log.callee_phone,
    },
  };

  if (log.accepted_by_name) {
    object.accepted_by = {
      name: log.accepted_by_name,
      extension_number: log.accepted_by_extension || undefined,
    };
  }

  if (log.owner_type || log.owner_name) {
    object.owner = {
      type: log.owner_type || 'callQueue',
      name: log.owner_name,
      extension_number: log.owner_extension || log.callee_extension,
    };
  }
...
/**
 * Fetch Zoom Phone call log metadata for Call Tracker manual import.
 */
export async function fetchZoomCallLogSummary(callId) {
  const id = String(callId ?? '').trim();
  if (!id) {
    return { ok: false, reason: 'missing_call_id', message: 'call_id is required' };
  }
  ...
  const attempts = [];
  for (const path of [`/phone/call_logs/${encodeURIComponent(id)}`, `/phone/call_history/${encodeURIComponent(id)}`]) {
    const res = await zoomApiGetWithFallback(path);
    attempts.push({ step: path, ...res });
    if (res.ok && res.data) {
      const log = normalizeCallLogRow(res.data, id);
      if (log.caller_phone || log.caller_name) {
        return { ok: true, log, attempts, zoom_source: path };
      }
    }
  }
  ...
  return {
    ok: false,
    reason: last?.http_status === 404 ? 'call_not_found' : last?.reason ?? 'zoom_api',
    message:
      last?.http_status === 404
        ? 'Zoom could not find this call ID. Copy it from Zoom Phone → Call History → call details.'
        : last?.error || 'Could not load call from Zoom.',
    attempts,
  };
}
