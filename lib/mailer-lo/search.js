 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 */
export async function searchMailerLeads(supabase, query, { limit = 25, loName = null } = {}) {
  ...
  if (exactCode) {
    ...
    if (exactRows?.length) {
      if (!loName) return exactRows;
      return exactRows.filter((row) => {
        if (!row.assigned_lo_name) return true;
        return normalizeLoName(row.assigned_lo_name) === normalizeLoName(loName);
      });
    }
  }
  ...
  if (loName) {
    queryBuilder = queryBuilder.or(quotedLoFilter(loName));
  }
