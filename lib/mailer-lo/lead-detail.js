...
function buildHistoryTimeline({ mailerLead, batch, events, transcripts, lead }) {
  ...
  if (mailerLead.mail_date) {
    items.push({
      ...
      detail: `Mail date: ${mailerLead.mail_date}${mailerLead.offer_expires ? ` · Expires ${mailerLead.offer_expires}` : ''}`,
    });
  }
  ...
  for (const transcript of transcripts) {
    const preview = String(transcript.transcript_text ?? '').slice(0, 200);
    items.push({
...
export function buildLeadScript(mailerLead, loName) {
  ...
  return renderCallScript(DEFAULT_MAILER_CALL_SCRIPT, {
    ...
    mtg_amount: mailerLead.mtg_amount || '—',
    new_rate: mailerLead.new_rate || '—',
    new_payment: mailerLead.new_total_payment || '—',
...
export function buildLeadBrief(mailerLead) {
  const lines = [
    ...
    `Mortgage balance (from mailer data): **${mailerLead.mtg_amount || '—'}**.`,
    `Offer rate / payment: **${mailerLead.new_rate || '—'}** / **${mailerLead.new_total_payment || '—'}**.`,
    `Mail date: **${mailerLead.mail_date || '—'}** · Expires: **${mailerLead.offer_expires || '—'}**.`,
  ];
