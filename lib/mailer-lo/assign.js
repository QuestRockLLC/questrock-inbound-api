...
export async function assignMailerLeadToLo(supabase, { referenceCode, mailerLeadId, loName, note }) {
  ...
  const shapeOk = Boolean(shapeAssignment.synced);

  return {
    ok: shapeOk || !existing.shape_lead_id,
    mailer_lead: updated,
    previous_lo: previousLo,
    assigned_at: now,
    lead_owner_id: depursLo ?? null,
    shape_assignment: shapeAssignment,
    message: shapeOk
      ? `Assigned to ${lo} in Supabase and Shape (lead_owner_id ${depursLo}).`
      : existing.shape_lead_id
        ? `Saved in Supabase only — Shape owner NOT updated: ${shapeAssignment.reason || shapeAssignment.error || 'unknown error'}`
        : `Saved in Supabase — no Shape lead id to assign owner.`,
  };
}
