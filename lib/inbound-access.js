...
/** Full unredacted transcript in Call Tracker (SSN/DOB visible). Default: Arsalan only. */
const DEFAULT_TRANSCRIPT_ADMIN_EMAILS = ['arashid@questrock.com'];
...
/** Call Tracker — Ray, Nikk, Jason, Bill, Arsalan only. Everyone else: no access. */
const DEFAULT_CALL_TRACKER_EMAILS = [
  'arashid@questrock.com',
  'nikksmith@questrock.com',
  'bmedley@questrock.com',
  'rayconway@questrock.com',
...
export function getTranscriptAdminEmails() {
  const fromEnv = process.env.TRANSCRIPT_ADMIN_EMAILS?.trim();
  const list = fromEnv
    ? fromEnv.split(',').map((e) => e.trim().toLowerCase()).filter(Boolean)
    : DEFAULT_TRANSCRIPT_ADMIN_EMAILS;
  return new Set(list);
}

/** Bypass transcript redaction in Call Tracker API (raw SSN/DOB visible). */
export function canViewUnredactedTranscripts(email) {
  if (!email) return false;
  return getTranscriptAdminEmails().has(String(email).trim().toLowerCase());
}

export function getCallTrackerEmails() {
  const allowed = new Set(DEFAULT_CALL_TRACKER_EMAILS.map((e) => e.toLowerCase()));
  const fromEnv = process.env.CALL_TRACKER_EMAILS?.trim();
  if (fromEnv) {
...
export function canAccessCallTracker(email) {
  if (!email) return false;
  return getCallTrackerEmails().has(String(email).trim().toLowerCase());
}
...
export function buildInboundUserProfile(email, displayName) {
  ...
  const transcriptAdmin = canViewUnredactedTranscripts(email);
  ...
  return {
    ...
    canViewUnredactedTranscripts: transcriptAdmin,
    loName,
    defaultPath: callTracker ? '/call-tracker/' : admin ? '/mailer-import/' : '/mailer-lo/',
  };
}
