...
/** In-app call tracker — executives + managers (Jason Friday, Bastian Johnston). */
...
/** Block list — no Call Tracker access even if admin/executive. */
const CALL_TRACKER_BLOCKED_EMAILS = new Set(['bmedley@questrock.com']);

export function canAccessCallTracker(email) {
  if (!email) return false;
  const normalized = String(email).trim().toLowerCase();
  if (CALL_TRACKER_BLOCKED_EMAILS.has(normalized)) return false;
  if (isInboundAdmin(normalized)) return true;
  return getCallTrackerEmails().has(normalized);
}
